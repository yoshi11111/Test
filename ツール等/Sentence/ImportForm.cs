    ...
    public partial class ImportForm : Form
    {
        ...
        private bool ReadFiles(out List<OneSheet> retList)
        {
            ...
            try
            {
                ...
                foreach (string subFolder in System.IO.Directory.GetDirectories(
    directory, "*", System.IO.SearchOption.AllDirectories))
                {
                    filePathList.AddRange(Directory.GetFiles(subFolder).ToList());

                }
                filePathList.AddRange(Directory.GetFiles(directory).ToList());

                List<string> excelFiles = (from file in filePathList
                                           where file.Contains(".xlsx") ||
                                           file.Contains(".xls")
                                           select file).ToList();
                List<string> textFiles = (from file in filePathList
